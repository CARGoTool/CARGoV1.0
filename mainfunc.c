...
int main(int argc,char *argv[])
{
     ...
     flog=fopen("AFSRlogsTDA.txt","a");/*Change ----  File is empty. flog is declared in structure.h*/
	// printf("Main --------------------\n"); 
	 ...
	 extern int sem_rec(struct actor_node *);
	 sem_rec(head_actor);	
    // printf("Conflict check--");
	 extern struct conflict* conflict_check(struct actor_node *);
	 struct conflict* conflict_list=conflict_check(head_actor);
    ...
    // printf("Conflict display--%d ",c);
     ...
     fprintf(flog,"\nOutput text is generated and stored in output_goal.xgrl\n");
